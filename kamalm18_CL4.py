...
def determine_device_update(devices):
    ...
    for i in devices:
        if update_required(i) == True:
            list_of_lists.append([i[0], determine_update(i)])
        else:
            pass
...
# ```
# 1) It was beneficial in the 3rd function so we could get the update info for all the devices as efficiently as possible. Although by adding another conditional statement to determine_update and determine_device_update, you could easily get the same results.
# 
# 2) It's possible to use the while loop instead and iterate through the length of the list "devices". I don't think theres a benefit in this case because a for loop does the exact same thing in way less lines of code. I think a while loop would be better for iterating through a list without sublists.
# 
# ```
