...
# ```
# 1) If the user enters a negative value: This will result in a value error because while computing the value of V, the sqyare root function is used. A negative d value will result in an negative argument for math.sqrt(). You cannot take the square root of a negative number, so a value error will occur.
# If the user enters non numeric characters: This will also result in a value error because the function float() cannot convert a non numeric value to a number. 
# 
# 2) Yes, it would change because python runs the code line by line. When it gets to the line that computes the value of V, the program will give a name error because the variable g was never defined beforehand. The program doesn't know what g is and can't perform the calculation.
# 
# ```
