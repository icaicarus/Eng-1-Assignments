...
def selection_sort(nums):
    ...
    # setting the first value to compare to the first element in the list
    for i in range(len(sorted_list)):
        min_index = i
        # setting the second value to compare as the element right after the first value in the list
        for j in range(i+1, len(sorted_list)):
            # select the minimum element in every iteration
            if sorted_list[j] < sorted_list[min_index]:
                min_index = j
         # swapping the element chosen with the one with the smaller value
...
def extract_temps(temps):
    temp_list = []
    for i in temps:
        # checking if item in the list can be converted to a float
        try:
            item = float(i)
        except ValueError:
            pass
        else:
            # adding item to a new list
...
def calculate_median(nums):
    size = len(nums)
    # checking if the length of the list is even or odd before calculating the median
    if (size%2) != 0:
        median = nums[size//2]
    else:
...
#********************************************
def main(temps):
    # checking if running any function would raise an error
    try:
        list_of_floats = extract_temps(temps)
        sorted_list = selection_sort(list_of_floats)
        median = calculate_median(sorted_list)
    except:
        return 'N/A'
    else:
        return median
...
# Input: [1,1,3,4,6,6,1,5]
# Output: [1,1,1,3,4,5,6,6]
# Expected Output: [1,1,1,3,4,5,6,6]
# Pass/Fail: Pass
# 
# ABNORMAL CASE
# Function: main(temps)
...
# ```
# 1) An integer (eg. 75987).
# 2) No because the main function will check for errors anyways.
# ```
